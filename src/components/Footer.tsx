    ...
    <footer className="bg-foreground text-primary-foreground">
      <div className="max-w-7xl mx-auto px-4 py-16">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10">
          <div>
            <h3 className="text-2xl font-bold mb-4 text-gold">{SITE_DATA.businessName}</h3>
          ...
          <div>
            ...
            <div className="space-y-3 text-sm text-primary-foreground/60">
              <a href={`tel:+91${SITE_DATA.contactNumber}`} className="flex items-center gap-2 hover:text-gold transition-colors">
                <Phone size={16} /> +91 {SITE_DATA.contactNumber}
              </a>

              <a
                href={SITE_DATA.instagramLink}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-2 hover:text-gold transition-colors"
              >
                <Instagram size={16} /> @{SITE_DATA.instagramLink.split('.com/')[1]?.replace('/', '')}
              </a>
              <a href={`https://www.google.com/maps?q=${encodeURIComponent(SITE_DATA.location)}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 hover:text-gold transition-colors">
                <MapPin size={16} /> {SITE_DATA.location}
              </a>
            </div>
          </div>
        </div>
        <div className="mt-12 pt-8 border-t border-primary-foreground/10 text-center text-sm text-primary-foreground/40">
          © {new Date().getFullYear()} {SITE_DATA.businessName} — Premium Bridal Makeup Artist. All rights reserved.
        </div>
